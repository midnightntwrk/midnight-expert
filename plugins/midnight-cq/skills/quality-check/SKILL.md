...
> **Last verified:** 2026-09-17 — `compact compile` CLI usage and package names/versions checked against the installed toolchain (compiler 0.31.1) + npm (`@openzeppelin/compact-simulator@0.3.1`, runtime 0.16.0).
...
| Contract compile | `compact compile --skip-zk` | All `.compact` files |
...
1. **Stale Compact compiler output** — types in `managed/` are regenerated by `compact compile`. If `.compact` source changed but `managed/` was not recompiled, tsc will report phantom type errors. Fix: run `compact compile --skip-zk` first, then re-run `tsc --noEmit`.
...
FAIL src/test/MyContract.test.ts
  ● transferOwnership › should fail when called by unauthorized
    ...
    Error: Ownable: caller is not the owner
      ...
      at ProxyHandler.<anonymous> (node_modules/@openzeppelin/compact-simulator/dist/...)
      at ...
...
| `tsc` errors about types in `managed/` | `compact compile --skip-zk && npx tsc --noEmit` — recompile contracts first |
